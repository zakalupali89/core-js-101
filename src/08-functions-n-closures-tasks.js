...
function getPolynom(...args) {
  return (x) => {
    ...
    return reversedArgs.reduce((acc, next, i) => acc + next * (x ** i), 0);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const argsToString = args.map((value) => JSON.stringify(value)).join(',');
    logFunc(`${funcName}(${argsToString}) starts`);
    const result = func(...args);
    logFunc(`${funcName}(${argsToString}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return fn.bind(undefined, ...args1);
}
